...
export default function CreateHRTicketModal({ open, ticket, onClose, onCreate }) {
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title="Create HR ticket"
      subtitle={`Ref: ${ticket?.id}`}
      size="md"
      footer={
        <>
          <ModalButton variant="secondary" onClick={onClose}>Cancel</ModalButton>
          <ModalButton variant="primary" onClick={handleSubmit}>Create HR ticket</ModalButton>
        </>
      }
    >
